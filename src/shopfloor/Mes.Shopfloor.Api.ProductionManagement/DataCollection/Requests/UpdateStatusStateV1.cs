...
using Mes.Shopfloor.Api.ProductionManagement.DataCollection.Requests.Factories;
...
internal static class UpdateStatusStateV1
{
        ...
        IProductionUnitStatusFactory factory) : ICommandHandler<Command>
    {
        public async Task<ICommandResponse> HandleAsync(Command request, CancellationToken cancellationToken)
        {
            ...
            var state = await context.GetStateByIdAsync(request.ProductionUnitId, cancellationToken);
            if (state == null)
                return CommandResponseFactory.BadRequest_400().Build();
            ...
            var statusState = StatusState.FromState(status.ProductionUnitId, state, request.OccurredAtUtc);
