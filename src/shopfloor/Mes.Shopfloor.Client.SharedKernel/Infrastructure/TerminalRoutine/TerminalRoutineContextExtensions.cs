...
public static class TerminalRoutineContextExtensions
{
    ...
    public static object? GetOrDefault(this ITerminalRoutineContext context, DataKey key, object? defaultValue = null)
    {
        return context.GetData(key)?.Data ?? defaultValue;
    }

    public static T? GetOrDefault<T>(this ITerminalRoutineContext context, DataKey key, T? defaultValue = default)
    {
        var data = context.GetData(key);
        if (data == null)
            return defaultValue;
